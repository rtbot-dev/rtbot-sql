...
std::vector<std::string> GraphBuilder::validate_session() const {
  std::vector<std::string> errors;
  validate_graph(operators_, connections_, "", errors);
  for (const auto& proto : prototypes_) {
    validate_graph(proto.operators, proto.connections,
...
std::string GraphBuilder::to_json() const {
  // Derive entry from the Input operator and outputs from the Output
  // operator, then delegate to to_json_session for the actual JSON assembly.
  ...
  if (!output_id.empty()) named_outputs[output_id] = {"o1"};
  return to_json_session(entry_id, named_outputs);
}

std::string GraphBuilder::to_json_session(
  ...
  if (!entry_op_id.empty()) {
    program["entryOperator"] = entry_op_id;
  }
  if (!named_outputs.empty()) {
    json out = json::object();
    for (const auto& [op_id, ports] : named_outputs) {
      json port_arr = json::array();
      for (const auto& p : ports) port_arr.push_back(p);
      out[op_id] = port_arr;
    }
    program["output"] = out;
  }
