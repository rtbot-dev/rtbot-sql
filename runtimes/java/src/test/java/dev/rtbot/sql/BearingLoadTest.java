...
public class BearingLoadTest {
    ...
    private static final int NUM_BURSTS = 100;
    ...
    public void bearingPresetThroughput() {
        ...
        try {
            ...
            int expectedRaw = NUM_BURSTS * NUM_BEARINGS * (SAMPLES_PER_BURST + 1);
            ...
            System.out.printf("  %-30s: %,8d rows%n", "vibration_raw (raw)", rawRows);
            System.out.printf("  %-30s: %,8d rows (should be 0)%n",
                              "vibration_moments (VIEW)", viewRows);
            System.out.printf("  %-30s: %,8d rows (expected %d)%n",
                              "rms_trend (MAT VIEW)", rmsRows, expectedPerMv);
            System.out.printf("  %-30s: %,8d rows (expected %d)%n",
                              "kurtosis_trend (MAT VIEW)", kurtosisRows, expectedPerMv);
            System.out.println();
            ...
            assertEquals("rms_trend rows", expectedPerMv, rmsRows);
            assertEquals("kurtosis_trend rows", expectedPerMv, kurtosisRows);
            assertEquals("raw stream rows", expectedRaw, rawRows);
