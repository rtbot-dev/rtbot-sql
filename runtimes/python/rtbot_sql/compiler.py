...
def compile_sql(sql: str, catalog_snapshot: Any, ts_units_per_second: int = 1_000_000) -> Any:
  return native.compile_sql(sql, catalog_snapshot, ts_units_per_second)
...
def compile_select_to_program(sql: str, catalog_snapshot: Any, ts_units_per_second: int = 1_000_000) -> Any:
  ...
  Returns a single CompilationResult (the last entry in the results list).
  """
  select_sql = sql.strip().rstrip(";")
  wrapped_sql = f"CREATE MATERIALIZED VIEW __rtbot_sql_tmp AS {select_sql}"
  compilation = native.compile_sql(wrapped_sql, catalog_snapshot, ts_units_per_second)
